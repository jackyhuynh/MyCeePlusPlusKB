...
// Helper function to print a vector (disabled for large sizes)
void printVector(const vector<int>& arr, int limit = 20) {
    if (arr.size() <= limit) {
        for (int val : arr) {
            cout << val << " ";
        }
        ...
        cout << "Vector size: " << arr.size() << " (printing first " << limit << " elements)..." << endl;
        for (int i = 0; i < limit; ++i) {
            cout << arr[i] << " ";
        }
        cout << "... (and " << arr.size() - limit << " more elements)" << endl;
    }
}
...
vector<int> generateRandomVector(int size) {
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> distrib(1, size * 10); // Adjust range as needed
...
int main() {
    int big_size = 10000; // Increased size for better comparison
    vector<int> data = generateRandomVector(big_size);
    vector<int> data_copy;

    cout << "Original array: ";
    printVector(data); // Print only a limited number of elements for large arrays
    cout << endl;
