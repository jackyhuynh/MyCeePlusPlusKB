...
This implementation uses a linked list with raw pointers to create a priority queue. Each node in the list contains the
data and its priority. The highest priority item is always at the front of the queue.
...
- Each node stores the item (`data`) and its associated `priority`. It also contains a `next` pointer to the next node
  in the queue.

2. **PriorityQueue Class**:

- **enqueue(const ItemType& newEntry, int priority)**: Adds a new entry into the priority queue based on its priority.
  The highest-priority item is always at the front.
...
This implementation uses a dynamic array to store the elements and their priorities. It maintains the priority queue
property by moving elements to their correct positions upon insertion.
